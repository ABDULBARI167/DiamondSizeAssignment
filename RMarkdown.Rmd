...
output of diamonds data smaller than 2.5 carats.
```{r, echo = FALSE}
...
smaller diamonds count; smaller or equal to 2.5 carats
``` {r, echo = FALSE}
# smaller diamonds count; smaller or equal to 2.5 carats
  nrow(smaller)  
```

count of diamond larger than 2.5 carats
``` {r, echo = FALSE}
 #count of diamond larger than 2.5 carats
  nrow(diamonds) -  nrow(smaller) 
```


```{r larger_diamonds,echo=FALSE}
# diamonds count; greater than 2.5 carats
larger_diamonds <- diamonds %>%
  filter(carat > 2.5)
```

distribution of diamonds larger than 2.5 carats
```{r, echo=FALSE}
#distribution of diamonds larger than 2.5 carats
larger_diamonds %>%
  ggplot(aes(carat)) + 
...
distribution of diamonds smaller or equal to 2.5 carats
```{r smaller_diamonds, echo = FALSE}
smaller %>% 
  ggplot(aes(carat)) + 
  geom_freqpoly(binwidth = 0.01)
```
